...
fn fetch_latest_version(fallback_version: &str) -> Version {
    run_fetch_version_from_github_api().unwrap_or_else(|| {
        if let Some(val) = fetch_version_from_git() {
            ...
            println!(
                "{}: You can still use blaze but won't be notified about new versions!",
                "Warning".yellow()
            );
...
async fn fetch_version_from_github_api() -> Option<Version> {
    let url = "https://api.github.com/repos/humblepenguinn/blaze/releases/latest";
    let client = reqwest::Client::new();
    let res = if let Ok(val) = client.get(url).header("User-Agent", "blaze").send().await {
        val
    } else {
        return None;
    };
